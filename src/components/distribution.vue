<template>
  <section style="margin-top:60px;">
    <el-row :gutter="4" type="flex" justify="center">
      <el-col :span="8" class="compare">
        <el-row>
          <el-input placeholder="Key in module code " v-model="leftwords">
            <el-button slot="append" icon="el-icon-search" @click="getAverage('left')"></el-button>
          </el-input>
        </el-row>
        <el-row class="search-results" :gutter="2" v-if="showRate">
          <el-col v-for="(item, index) in datas.leftresults" v-bind:key="index" :span=" 8">
            <h5>{{item.name}}</h5>
            <el-rate
              v-model="item.score"
              disabled
        ...
        <el-row class="bar-charts">
          <el-col :span="20" class="charts">
            <div class="line-chart" id="left"></div>
          </el-col>
        </el-row>
      </el-col>
      <el-col :span="8" class="compare" :offset="1">
        <el-row offset="2">
          <el-input placeholder="Key in module code" v-model="rightwords">
            <el-button slot="append" icon="el-icon-search" @click="getAverage(right)"></el-button>
          </el-input>
        </el-row>
        <el-row class="search-results" :gutter="2" v-if="showright">
          <el-col v-for="(item, index) in datas.rightresults" v-bind:key="index" :span="8">
            <h5>{{item.name}}</h5>
            <el-rate
              v-model="item.score"
              disabled
        ...
        <el-row class="bar-charts">
          <el-col :span="20" class="charts">
            <div class="line-chart" id="right"></div>
          </el-col>
        </el-row>
      </el-col>
    </el-row>
