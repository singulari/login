<template >
  <section class="enrolment">
    <el-row>
      ...
      >
        <el-input
          placeholder="BT3103 Application Systems Development for Business Analytics" //default text in the search background
          v-model="keywords" //input your search module 
          class="input-with-select"
          @keypress="getAverage"
        >
          <el-button
            slot="append"
            icon="el-icon-search" //search button 
            @click="getAverage" //gives ratings 
          ></el-button>
        </el-input>
      </el-col>
    </el-row>
    <el-row class="previous">
      <el-col
        :span="16"
        :offset="2"
        class="previous-search" //creates list of previous searches
      >
        <ul>
          <li
    ...
    >
      <el-col
        v-for="(item, index) in datas.results" //ratings for module difficulty, workload, and professor 
        v-bind:key="index"
        :span=" 6"
      >
        <h3>{{item.name}}</h3>
    ...
    <el-row class="bar-charts"> //for score distribution 
      <el-col
        :span="20"
        class="charts"
      >
...
export default {
  ...
  data() {
    return {
      ...
      option: {
        title: {
          text: "Academic Year 18/19 Semester 2", //title for bar chart 
          textAlign: "center",
          left: "center"
        },
